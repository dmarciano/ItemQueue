...
namespace SMC.Utilities.Queues
{
    public class FunctionQueue<T, TResult> : IDisposable
    {
        ...
        private Action<TResult> _Action;
        private Action<T, TResult> _Callback;
        ...
        /// </summary>
        /// <param name="function">The function to call with the current item whose result is used to call the <paramref name="action"/> on.</param>
        /// <param name="action">The action to call on each item in the queue.</param>
        public FunctionQueue(Func<T, TResult> function, Action<TResult> action) : this(function, action, null, string.Empty) { }

        /// <summary>
        /// Create a new function queue with the specified function, action, and callback.
        /// </summary>
        ///  <param name="function">The function to call with the current item whose result is used to call the <paramref name="action"/> on.</param>
        /// <param name="action">The action to call on each item in the queue.</param>
        ...
        public FunctionQueue(Func<T, TResult> function, Action<TResult> action, Action<T, TResult> callback) : this(function, action, callback, string.Empty) { }

        /// <summary>
        /// Create a new function queue with the specified function, action, and queue name.
        /// </summary>
        ///  <param name="function">The function to call with the current item whose result is used to call the <paramref name="action"/> on.</param>
        /// <param name="action">The action to call on each item in the queue.</param>
        /// <param name="queueName">The name for this queue.</param>
        /// <remarks>The <paramref name="queueName"/> is also used as the name of the background thread.</remarks>
        public FunctionQueue(Func<T, TResult> function, Action<TResult> action, string queueName) : this(function, action, null, queueName) { }

        /// <summary>
        /// Create a new predicate queue with the specified function, action, callback, and queue name.
        /// </summary>
        ///  <param name="function">The function to call with the current item whose result is used to call the <paramref name="action"/> on.</param>
        /// <param name="action">The action to call on each item in the queue.</param>
        /// <param name="callback">The callback that will be called for each item that is successfully processed without an exception.</param>
        /// <param name="queueName">The name for this queue.</param>
        /// <remarks>The <paramref name="queueName"/> is also used as the name of the background thread.</remarks>
        public FunctionQueue(Func<T, TResult> function, Action<TResult> action, Action<T, TResult> callback, string queueName)
        {
            _Function = function;
            _Action = action;
            _Callback = callback;
        ...
        /// <exception cref="NoFunctionException">No function was specified in the constructor or using the <see cref="SetFunction(Func{T, TResult})"/> method.</exception>
        /// <exception cref="NoActionException">No action was specified in the constructor or using the <see cref="SetAction(Action{TResult})"/> method.</exception>
        ...
        {
            if (null == _Function) throw new NoFunctionException("A function must be specified before the queue can start processing items.");
        ...
        /// <summary>
        /// Set the function whose result is used to call the specified action.
        /// </summary>
        /// <param name="function">The function to call with the current item whose result is used to call the action on.</param>
        public void SetFunction(Func<T, TResult> function)
        {
            _Function = function ?? throw new ArgumentNullException(nameof(function), "Function cannot be null.");
        }
        ...
        public void SetAction(Action<TResult> action)
        {
        ...
        public void SetCallback(Action<T, TResult> callback)
        {
        ...
        /// </summary>
        /// <remarks>To enable callbacks again, call the <see cref="SetCallback(Action{T, TResult})"/> method.</remarks>
                ...
                if (CancellationRequested)
                {
                    if (_ProcessRemainingItems)
                    {
                        foreach (var i in _Queue)
                        {
                            ...
                            {
                                var result = _Function(currentItem);
                                _Action(result);
                                OnActionCompleted(currentItem, true, string.Empty);
                                _Callback?.Invoke(currentItem, result);
                            }
                            else
                            {
